...
async function getPetData(reportId: string): Promise<PetReport | null> {
  try {
    ...
    const pet = combinedPets.find(
      (p) => p.report_id.toString() === reportId && p.report_type === 1
    );
    if (!pet) return null;
        ...
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
          <div className="relative">
            ...
            {pet.reward && (
              ...
              >
                <span className="inline-block animate-pulse">💰</span> REWARD: {pet.reward}
              </Badge>
            )}
          </div>
          ...
          <div className="p-4 md:p-6 lg:p-8">
            <div className="flex flex-col md:flex-row justify-between items-start mb-4 md:mb-6">
              <div>
                <h1 className="text-xl md:text-2xl lg:text-3xl font-bold mb-2">{pet.name}</h1>
            ...
            <div className="bg-red-50 border border-red-100 rounded-xl p-3 md:p-4 mb-4 md:mb-6">
              ...
              <div className="grid grid-cols-1 gap-3">
                <div className="flex items-start">
                  <Calendar size={16} className="mr-2 text-red-600 mt-1 flex-shrink-0" />
                  <div className="min-w-0">
                    <p className="font-medium text-gray-700 text-sm md:text-base">Date Lost</p>
                    <p className="text-sm md:text-base break-words">{pet.date_lost || pet.date}</p>
                  </div>
                </div>
                <div className="flex items-start">
                  <MapPin size={16} className="mr-2 text-red-600 mt-1 flex-shrink-0" />
                  <div className="min-w-0">
                    <p className="font-medium text-gray-700 text-sm md:text-base">Last Seen</p>
                    <p className="text-sm md:text-base break-words">
                      {pet.nearest_address_last_seen || pet.location || "Unknown"}
                    </p>
                  </div>
                </div>
                {pet.distinguishing_features && (
                  <div className="flex items-start">
                    <Info size={16} className="mr-2 text-red-600 mt-1 flex-shrink-0" />
                    <div className="min-w-0">
                      <p className="font-medium text-gray-700 text-sm md:text-base">Features</p>
                      <p className="text-sm md:text-base break-words">{pet.distinguishing_features}</p>
                    </div>
                  </div>
                )}
                {pet.reward && (
                  <div className="flex items-start">
                    <DollarSign size={16} className="mr-2 text-red-600 mt-1 flex-shrink-0" />
                    <div className="min-w-0">
                      <p className="font-medium text-gray-700 text-sm md:text-base">Reward</p>
                      <p className="text-sm md:text-base break-words">{pet.reward}</p>
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="space-y-6">
              <div>
                <h2 className="text-lg md:text-xl font-bold mb-3">Pet Details</h2>
                <div className="grid grid-cols-2 gap-x-3 gap-y-2 text-sm md:text-base">
                  ...
                  <div>
                    ...
                    <p className="font-semibold">{speciesMap[pet.species] || "Unknown"}</p>
                  </div>
                  {pet.breed_id && (
                    <div>
                  ...
                  {pet.sex && (
                    <div>
                      ...
                      <p className="font-semibold">{sexMap[pet.sex] || "Unknown"}</p>
                    </div>
                  )}
                  {pet.size && (
                    <div>
                      <p className="text-gray-500 text-xs md:text-sm">Size</p>
                      <p className="font-semibold">{sizeMap[pet.size] || "Unknown"}</p>
                    </div>
                  )}
                  {pet.distinguishing_features && (
                    <div className="col-span-2">
                      <p className="text-gray-500 text-xs md:text-sm">Distinctive Features</p>
                      <p className="font-semibold break-words">{pet.distinguishing_features}</p>
                    </div>
                  )}
                </div>
                {pet.description && (
                  <div className="mt-4">
                    ...
                    <p className="text-gray-600 text-sm md:text-base">{pet.description}</p>
                  </div>
                )}
              </div>
                ...
                <Card className="bg-gray-50 border-0 rounded-xl mb-4">
                  <CardContent className="p-4">
                    <div className="flex items-center mb-3">
                      <User size={16} className="text-[#4eb7f0] mr-2 flex-shrink-0" />
                      <div className="min-w-0">
                        <p className="text-gray-500 text-xs md:text-sm">Owner Name</p>
                        <p className="font-medium text-sm md:text-base break-words">{pet.owner_name}</p>
                      </div>
                    </div>
                    <div className="flex items-center mb-3">
                      <Phone size={16} className="text-[#4eb7f0] mr-2 flex-shrink-0" />
                      <div className="min-w-0">
                        <p className="text-gray-500 text-xs md:text-sm">Phone Number</p>
                    ...
                    <div className="flex items-center">
                      <Mail size={16} className="text-[#4eb7f0] mr-2 flex-shrink-0" />
                      <div className="min-w-0">
                        <p className="text-gray-500 text-xs md:text-sm">Contact Email</p>
                        <p className="font-medium text-sm md:text-base break-words">
